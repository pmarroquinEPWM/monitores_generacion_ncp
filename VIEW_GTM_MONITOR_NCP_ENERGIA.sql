CREATE VIEW GTM_MONITOR_NCP_ENERGIA AS
(
SELECT A2.FECHA, A2.id_central,A2.GENERADOR,A2.TECNOLOGIA, A2.MW_REAL, IF(B2.MW_NCP IS NOT NULL, B2.MW_NCP, 0) AS 'MW_NCP',B2.NEXT_DAY, B2.ESCENARIO,  B2.version  FROM
    ...
    WHERE FECHA BETWEEN DATE_ADD(CURDATE(), INTERVAL -1 MONTH) AND CURDATE()
    ...
    (SELECT B1.ETAPA FECHA, SUM(MWH) MW_NCP, DIC.id_central, B1.NEXT_DAY, B1.ESCENARIO,  SIM.version FROM bi.GTM_CARGA_NCP B1,
                                                                                                       ...
                                                                                                       (select id_simulacion,id_escenario,fecha, version from SIMULACIONES_NCP  WHERE FECHA BETWEEN DATE_ADD(CURDATE(), INTERVAL -1 MONTH) AND CURDATE()) SIM
    WHERE ETAPA BETWEEN DATE_ADD(CURDATE(), INTERVAL -1 MONTH) AND CURDATE()
